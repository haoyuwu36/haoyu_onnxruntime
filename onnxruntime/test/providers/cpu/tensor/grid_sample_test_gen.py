...
dtype = {"float":torch.float, "float16":torch.float16}
    ...
    for mode in modes:
        for padding_mode in padding_modes:
            for align_corners in align_corners_options:
                for onnxType, torchType in enumerate(dtype):
                    for ndim in [4, 5]:
                        ...
                        input_tensor = torch.randn(*input_shape).to(torch.float)
                        ...
                        print(f"std::initializer_list<{onnxType}> X_data { X_data_str };")
                        print(f"std::initializer_list<int64_t> Grid_shape {{ {', '.join(map(str, grid_shape))} }};")
                        print(f"std::initializer_list<{onnxType}> Grid_data { Grid_data_str };")
                        print(f"std::initializer_list<int64_t> Y_shape {{ {', '.join(map(str, Y_shape))} }};")
                        print(f"std::initializer_list<{onnxType}> Y_data { Y_data_str };")
